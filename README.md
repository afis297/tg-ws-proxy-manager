...
- Пункт `13) Переустановить` — каталог проекта удаляется и клонируется заново,
  конфиг и secret сохраняются. Есть защита от удаления опасных путей.
- Пункт `14) Удалить` — удаляет проект, конфиг и логи — только по запросу.
- `tgws purge` — полная очистка: всё выше плюс автозапуск, алиас и сам менеджер.
