...
- [Команды и меню](docs/commands.md) — весь CLI и все 31 пункт меню.
...
склонирует `tg-ws-proxy` в `~/tg-ws-proxy` (ветка `v1.10.0`),
создаст конфиг и сгенерирует secret.

Короткая команда после установки (пункт `29`, алиас):
...
4. Пункт `10) Раздача и ссылки` — включить `lan`, если нужен доступ
   с других устройств в той же сети.
5. Пункт `19) Автозапуск` — включить, чтобы переживало перезагрузку.
6. Пункт `30) Настройки батареи` — исключить Termux из оптимизации,
