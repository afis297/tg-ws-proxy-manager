...
- `purge` — полная очистка: стоп прокси и watchdog, снятие обоих автозапусков, удаление ядра, конфигов, алиаса и (по запросу) самого менеджера. Пакеты Termux не трогает. Два подтверждения, отмены нет.
