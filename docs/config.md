...
| `REPO_BRANCH` | `v1.10.0` | зафиксированная версия (защита от supply chain) |
...
| `RESTART_EVERY_H` | `0` | плановый перезапуск каждые N часов, `0` — выкл (нужен включённый watchdog) |
...
- `detect-ip` / пункт `11`: определение IP идёт каскадом —
